...
# hidden W, b
W_encoder_input_hidden = weight_variable([input_dim, hidden_encoder_dim])  # [1024, 400]
...
# sample latent variable z, representation
...
# hidden reconstruction W, b
W_decoder_hidden_reconstruction = weight_variable([hidden_decoder_dim, input_dim])
b_decoder_hidden_reconstruction = bias_variable([input_dim])
l2_loss += tf.nn.l2_loss(W_decoder_hidden_reconstruction)

# KLD is associated with encoder mu and logvar
KLD = -0.5 * tf.reduce_sum(1 + logvar_encoder - tf.pow(mu_encoder, 2) - tf.exp(logvar_encoder), reduction_indices=1)

# BCE is associated with reconstruction x and input x
...
n_steps = int(1e6)
batch_size = 128  # in every train data : 10000
